...
def approx_mean_conservative(I, alpha):
    global BG_mean
...
def approx_median_conservative(I):
    global BG_median
    IG = cv2.cvtColor(I, cv2.COLOR_BGR2GRAY).astype('uint8')
    ...
    IG_MEDIAN_BIN = (255*((IG_MEDIAN > 170) | (IG_MEDIAN < 90))).astype('uint8')
    ...
    BG_median += ((BG_median < IG) & IG_MEDIAN_BIN).astype('uint8')
    BG_median -= ((BG_median > IG) & IG_MEDIAN_BIN).astype('uint8')
